...
/**
 * Get any active timer for a user (across all tickets)
 */
function get_user_active_timer($user_id) {
    if (!ticket_time_table_exists()) {
        return null;
    }
    try {
        return db_fetch_one("SELECT tte.*, t.title as ticket_title
                             FROM ticket_time_entries tte
                             LEFT JOIN tickets t ON tte.ticket_id = t.id
                             WHERE tte.user_id = ? AND tte.ended_at IS NULL
                             ORDER BY tte.started_at DESC LIMIT 1", [$user_id]);
    } catch (Exception $e) {
        return null;
    }
}
 ...
 */
function start_ticket_timer($ticket_id, $user_id) {
    if (!ticket_time_table_exists()) {
        return false;
    }
    ...
    try {
        return db_insert('ticket_time_entries', [
            'ticket_id' => $ticket_id,
            'user_id' => $user_id,
...
/**
 * Stop a timer on a ticket
 */
function stop_ticket_timer($ticket_id, $user_id, $summary = null) {
    if (!ticket_time_table_exists()) {
        return false;
    }
    ...
    $duration = max(1, (int)floor(($ended->getTimestamp() - $started->getTimestamp()) / 60));
    ...
    try {
        ...
        return db_update('ticket_time_entries', $data, 'id = ?', [$timer['id']]);
    } catch (Exception $e) {
        return false;
    }
}
