...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['download_md_template'])) {
        $template_key = trim((string) ($_POST['md_template_key'] ?? ''));
        $template_content = ticket_import_get_template_content($template_key);
        $template_filename = ticket_import_get_template_filename($template_key);

        if ($template_content === null || $template_filename === null) {
            flash(t('Invalid .md template selection.'), 'error');
            redirect('admin', ['section' => 'settings', 'tab' => 'system']);
        }

        $settings_audit('md_template_download', [
        ...
        header('Content-Disposition: attachment; filename="' . basename($template_filename) . '"');
    ...
    if (isset($_POST['generate_api_token'])) {
        ...
        if ($token_name === '') {
            ...
            $user = current_user();
            $result = generate_api_token($user['id'], $token_name);
            $_SESSION['new_api_token'] = $result['token'];
            $settings_audit('api_token_generated', ['token_name' => $token_name]);
            flash(t('Token generated successfully. Copy it now — it won\'t be shown again:'), 'success');
        }
    ...
    if (isset($_POST['revoke_api_token'])) {
        ...
        if ($token_id > 0) {
            ...
            $settings_audit('api_token_revoked', ['token_id' => $token_id]);
    ...
    if (isset($_POST['delete_api_token'])) {
        ...
        if ($token_id > 0) {
            ...
            $settings_audit('api_token_deleted', ['token_id' => $token_id]);
            flash(t('Token deleted.'), 'success');
        }
        redirect('admin', ['section' => 'settings', 'tab' => 'api']);
    }
...
<div>
    ...
    <div class="flex flex-wrap gap-0.5 mb-2 p-0.5 rounded-lg w-fit" style="background: var(--surface-secondary);">
        ...
        <a href="<?php echo url('admin', ['section' => 'settings', 'tab' => 'api']); ?>"
            class="px-3 py-1.5 rounded-md text-xs font-medium <?php echo $tab === 'api' ? 'shadow text-blue-600' : ''; ?>"
            style="<?php echo $tab === 'api' ? 'background: var(--bg-primary);' : 'color: var(--text-secondary);'; ?>">
            <?php echo e(t('API')); ?>
        </a>
    ...
    <?php elseif ($tab === 'system'): ?>
        ...
        <div class="card card-body mt-2">
            <h3 class="text-xs font-semibold uppercase tracking-wide mb-1.5" style="color: var(--text-muted);">
                <?php echo e(t('Markdown import templates')); ?>
            </h3>
            <p class="text-xs mb-2" style="color: var(--text-muted);">
                <?php echo e(t('Use these English templates to import new tickets or worklog updates from .md files.')); ?>
            </p>
            <div class="space-y-1.5">
                <?php foreach (ticket_import_get_available_templates() as $template_key => $template_info): ?>
                    <div class="flex items-center justify-between gap-2 px-2.5 py-1.5 rounded-lg border"
                        ...
                        <div class="min-w-0">
                            <div class="text-xs font-medium truncate" style="color: var(--text-primary);">
                                <?php echo e($template_info['label'] ?? $template_key); ?>
                            </div>
                            <?php if (!empty($template_info['description'])): ?>
                                <div class="text-[11px] truncate" style="color: var(--text-muted);">
                                    <?php echo e($template_info['description']); ?>
                                </div>
                            <?php endif; ?>
                        </div>
                        <form method="post" class="flex-shrink-0">
                            <?php echo csrf_field(); ?>
                            <input type="hidden" name="md_template_key" value="<?php echo e($template_key); ?>">
                            <button type="submit" name="download_md_template" class="btn btn-secondary btn-sm text-xs">
                                <?php echo get_icon('download', 'mr-1'); ?>         <?php echo e(t('Download')); ?>
                            </button>
                        </form>
                    </div>
                <?php endforeach; ?>
    ...
    <?php elseif ($tab === 'api'): ?>
        ...
        <div class="card card-body">
            <h3 class="text-xs font-semibold uppercase tracking-wide mb-3" style="color: var(--text-muted);">
                <?php echo e(t('API tokens')); ?>
            </h3>
            <p class="text-sm mb-4 break-words" style="color: var(--text-secondary);">
                <?php echo e(t('API tokens allow external tools and AI agents to interact with your helpdesk. Each token inherits the permissions of the user it belongs to.')); ?>
            </p>

            <?php
            // Show newly generated token (once only)
            $new_token = $_SESSION['new_api_token'] ?? null;
            unset($_SESSION['new_api_token']);
            if ($new_token):
                ?>
                <div class="mb-4 p-3 rounded-lg bg-green-50 border border-green-200 overflow-hidden">
                    <p class="text-sm font-medium text-green-800 mb-2">
                        <?php echo e(t('Your new API token (copy now — it won\'t be shown again):')); ?>
                    </p>
                    <div class="flex items-center gap-2">
                        <code id="new-token-value"
                            class="flex-1 min-w-0 px-3 py-2 rounded border border-green-300 text-sm font-mono text-green-900 select-all break-all"
                            style="background: var(--bg-primary);"><?php echo e($new_token); ?></code>
                        <button type="button"
                            onclick="navigator.clipboard.writeText(document.getElementById('new-token-value').textContent).then(()=>{this.textContent='<?php echo e(t('Copied!')); ?>';setTimeout(()=>{this.textContent='<?php echo e(t('Copy')); ?>'},2000)})"
                            class="btn btn-sm bg-green-600 text-white hover:bg-green-700 whitespace-nowrap shrink-0">
                            <?php echo e(t('Copy')); ?>
                        </button>
                    </div>
                </div>
            ...
            <form method="post" class="flex items-end gap-3 mb-6">
                <?php echo csrf_field(); ?>
                <div class="flex-1 max-w-xs">
                    <label class="block text-sm font-medium mb-1"
                        style="color: var(--text-secondary);"><?php echo e(t('Token name')); ?></label>
                    <input type="text" name="token_name" required maxlength="100"
                        placeholder="<?php echo e(t('e.g. Claude Agent, CI/CD Pipeline')); ?>" class="form-input w-full">
                </div>
                <button type="submit" name="generate_api_token" class="btn btn-primary whitespace-nowrap">
                    <?php echo get_icon('key', 'w-4 h-4 mr-1'); ?>
                    <?php echo e(t('Generate token')); ?>
                </button>
            </form>

            <!-- Token list -->
            <?php
            $api_tokens = get_all_api_tokens();
            if (empty($api_tokens)):
                ?>
                <p class="text-sm italic" style="color: var(--text-muted);"><?php echo e(t('No tokens yet.')); ?></p>
            <?php else: ?>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b" style="border-color: var(--border-light);">
                                <th class="text-left py-2 px-2 font-medium" style="color: var(--text-secondary);">
                                    <?php echo e(t('Name')); ?>
                                </th>
                                <th class="text-left py-2 px-2 font-medium" style="color: var(--text-secondary);">
                                    <?php echo e(t('Token prefix')); ?>
                                </th>
                                <th class="text-left py-2 px-2 font-medium" style="color: var(--text-secondary);">
                                    <?php echo e(t('User')); ?>
                                </th>
                                ...
                                <th class="text-left py-2 px-2 font-medium" style="color: var(--text-secondary);">
                                    <?php echo e(t('Last used')); ?>
                                </th>
                                <th class="text-left py-2 px-2 font-medium" style="color: var(--text-secondary);">
                                    <?php echo e(t('Status')); ?>
                                </th>
                                <th class="text-right py-2 px-2 font-medium" style="color: var(--text-secondary);">
                                    <?php echo e(t('Actions')); ?>
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($api_tokens as $tk): ?>
                                <tr class="border-b tr-hover" style="border-color: var(--border-light);">
                                    <td class="py-2 px-2 font-medium"><?php echo e($tk['name']); ?></td>
                                    <td class="py-2 px-2"><code class="text-xs px-1.5 py-0.5 rounded"
                                            style="background: var(--surface-secondary);"><?php echo e($tk['token_prefix']); ?>...</code>
                                    </td>
                                    <td class="py-2 px-2">
                                        <?php echo e(trim(($tk['first_name'] ?? '') . ' ' . ($tk['last_name'] ?? ''))); ?>
                                    </td>
                                    <td class="py-2 px-2" style="color: var(--text-muted);">
                                        <?php echo e(date('Y-m-d', strtotime($tk['created_at']))); ?>
                                    </td>
                                    <td class="py-2 px-2" style="color: var(--text-muted);">
                                        <?php echo $tk['last_used_at'] ? e(date('Y-m-d H:i', strtotime($tk['last_used_at']))) : '<span class="italic">' . e(t('Never')) . '</span>'; ?>
                                    </td>
                                    <td class="py-2 px-2">
                                        <?php if ($tk['is_active']): ?>
                                            <span
                                                class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800"><?php echo e(t('Active')); ?></span>
                                        <?php else: ?>
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium"
                                                style="background: var(--surface-secondary); color: var(--text-secondary);"><?php echo e(t('Revoked')); ?></span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="py-2 px-2 text-right">
                                        <?php if ($tk['is_active']): ?>
                                            <form method="post" class="inline"
                                                onsubmit="return confirm('<?php echo e(t('Revoke this token?')); ?>')">
                                                <?php echo csrf_field(); ?>
                                                <input type="hidden" name="token_id" value="<?php echo (int) $tk['id']; ?>">
                                                <button type="submit" name="revoke_api_token"
                                                    class="text-yellow-600 hover:text-yellow-800 text-xs font-medium"><?php echo e(t('Revoke')); ?></button>
                                            </form>
                                        <?php else: ?>
                                            <form method="post" class="inline"
                                                onsubmit="return confirm('<?php echo e(t('Delete this token permanently?')); ?>')">
                                                <?php echo csrf_field(); ?>
                                                <input type="hidden" name="token_id" value="<?php echo (int) $tk['id']; ?>">
                                                <button type="submit" name="delete_api_token"
                                                    class="text-red-600 hover:text-red-800 text-xs font-medium"><?php echo e(t('Delete')); ?></button>
                                            </form>
                                        <?php endif; ?>
                                    </td>
                                </tr>
        ...
        <div class="card card-body mt-4">
            <details>
                <summary class="cursor-pointer text-xs font-semibold uppercase tracking-wide mb-2"
                    style="color: var(--text-muted);">
                    <?php echo e(t('API documentation')); ?>
                </summary>
                <div class="mt-3 text-sm space-y-4" style="color: var(--text-secondary);">
                    <p><?php echo e(t('All agent API endpoints use Bearer token authentication. Include the token in the Authorization header:')); ?>
                    </p>
                    <pre class="text-green-400 p-3 rounded-lg text-xs overflow-x-auto"
                        style="background: var(--surface-tertiary);">Authorization: Bearer ahd_your_token_here</pre>

                    <p class="font-medium" style="color: var(--text-primary);"><?php echo e(t('Base URL')); ?>:</p>
                    <pre class="p-2 rounded text-xs"
                        style="background: var(--surface-secondary);"><?php echo e((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' ? 'https' : 'http') . '://' . ($_SERVER['HTTP_HOST'] ?? 'your-domain.com') . dirname($_SERVER['SCRIPT_NAME'])); ?>/index.php?page=api&amp;action=</pre>

                    <div class="space-y-3">
                        <div>
                            <p class="font-medium" style="color: var(--text-primary);">GET <code
                                    class="text-xs px-1 rounded"
                                    style="background: var(--surface-secondary);">agent-me</code></p>
                            <p class="text-xs" style="color: var(--text-muted);">
                                <?php echo e(t('Returns the authenticated user\'s info.')); ?>
                            </p>
                        </div>
                        <div>
                            <p class="font-medium" style="color: var(--text-primary);">GET <code
                                    class="text-xs px-1 rounded"
                                    style="background: var(--surface-secondary);">agent-list-statuses</code></p>
                            <p class="text-xs" style="color: var(--text-muted);">
                                <?php echo e(t('Returns all available ticket statuses.')); ?>
                            </p>
                        </div>
                        <div>
                            <p class="font-medium" style="color: var(--text-primary);">GET <code
                                    class="text-xs px-1 rounded"
                                    style="background: var(--surface-secondary);">agent-list-priorities</code></p>
                            <p class="text-xs" style="color: var(--text-muted);">
                                <?php echo e(t('Returns all available ticket priorities.')); ?>
                            </p>
                        </div>
                        <div>
                            <p class="font-medium" style="color: var(--text-primary);">GET <code
                                    class="text-xs px-1 rounded"
                                    style="background: var(--surface-secondary);">agent-list-users</code> <span
                                    style="color: var(--text-muted);">?role=agent</span></p>
                            <p class="text-xs" style="color: var(--text-muted);">
                                <?php echo e(t('Returns users. Optional role filter: admin, agent, user.')); ?>
                            </p>
                        </div>
                        <div>
                            <p class="font-medium" style="color: var(--text-primary);">GET <code
                                    class="text-xs px-1 rounded"
                                    style="background: var(--surface-secondary);">agent-list-tickets</code> <span
                                    style="color: var(--text-muted);">?status=open&amp;limit=20&amp;search=keyword</span>
                            </p>
                            <p class="text-xs" style="color: var(--text-muted);">
                                <?php echo e(t('List/search tickets. Filters: status, priority, search, user_id, assignee_id, sort, limit, offset.')); ?>
                            </p>
                        </div>
                        <div>
                            <p class="font-medium" style="color: var(--text-primary);">GET <code
                                    class="text-xs px-1 rounded"
                                    style="background: var(--surface-secondary);">agent-get-ticket</code> <span
                                    style="color: var(--text-muted);">?hash=abc123</span></p>
                            <p class="text-xs" style="color: var(--text-muted);">
                                <?php echo e(t('Get full ticket detail including comments and time entries. Use hash or id parameter.')); ?>
                            </p>
                        </div>
                        <div>
                            <p class="font-medium" style="color: var(--text-primary);">POST <code
                                    class="text-xs px-1 rounded"
                                    style="background: var(--surface-secondary);">agent-create-ticket</code></p>
                            <p class="text-xs" style="color: var(--text-muted);">
                                <?php echo e(t('Create a ticket. JSON body: title (required), description, priority_id, status_id, user_id, assignee_id, due_date, tags.')); ?>
                            </p>
                        </div>
                        <div>
                            <p class="font-medium" style="color: var(--text-primary);">POST <code
                                    class="text-xs px-1 rounded"
                                    style="background: var(--surface-secondary);">agent-add-comment</code></p>
                            <p class="text-xs" style="color: var(--text-muted);">
                                <?php echo e(t('Add a comment. JSON body: ticket_hash or ticket_id, content (required), is_internal (boolean).')); ?>
                            </p>
                        </div>
                        <div>
                            <p class="font-medium" style="color: var(--text-primary);">POST <code
                                    class="text-xs px-1 rounded"
                                    style="background: var(--surface-secondary);">agent-update-status</code></p>
                            <p class="text-xs" style="color: var(--text-muted);">
                                <?php echo e(t('Change ticket status. JSON body: ticket_hash or ticket_id, status_id or status (name).')); ?>
                            </p>
                        </div>
                        <div>
                            <p class="font-medium" style="color: var(--text-primary);">POST <code
                                    class="text-xs px-1 rounded"
                                    style="background: var(--surface-secondary);">agent-log-time</code></p>
                            <p class="text-xs" style="color: var(--text-muted);">
                                <?php echo e(t('Log a time entry. JSON body: ticket_hash or ticket_id, duration_minutes (required), summary, is_billable.')); ?>
                            </p>
                        </div>
                    </div>
                </div>
