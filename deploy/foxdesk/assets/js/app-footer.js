...
function confirmDelete(message) {
    return confirm(message || (window.appConfig && window.appConfig.deleteConfirmMsg) || 'Are you sure?');
}
...
function playNotificationSound(type) {
    ...
    try {
        ...
        var freqMap = { success: 880, warning: 560, error: 320, info: 640 };
        oscillator.type = 'sine';
        oscillator.frequency.value = freqMap[type] || 640;
...
document.addEventListener('click', function(event) {
    ...
    if (event.target.closest('a, button, input, select, textarea, .bulk-checkbox')) return;
...
function showInlineLoading(container, message) {
    ...
    container.innerHTML = '<span class="loading-spinner loading-spinner-sm"></span>'
        + (message ? ' <span class="text-gray-500">' + message + '</span>' : '');
}
