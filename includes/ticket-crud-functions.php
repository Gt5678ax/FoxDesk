 ...
 */
function count_tickets_by_status() {
    $where_clauses = ["1=1"];
    $params = [];
    ...
    try {
        ...
        if ($check) {
            $where_clauses[] = "(t.is_archived IS NULL OR t.is_archived = 0)";
        }
    } catch (Exception $e) {
    ...
    if ($current_user && $current_user['role'] === 'agent') {
        $permissions = get_user_permissions($current_user['id']);

        switch ($permissions['ticket_scope']) {
            case 'assigned':
                if ($has_ticket_access) {
                    $where_clauses[] = "(t.assignee_id = ? OR EXISTS (SELECT 1 FROM ticket_access ta WHERE ta.ticket_id = t.id AND ta.user_id = ?))";
                    $params[] = $current_user['id'];
                    $params[] = $current_user['id'];
                } else {
                    $where_clauses[] = "t.assignee_id = ?";
                    $params[] = $current_user['id'];
                }
                break;
            case 'organization':
                ...
                if (!empty($org_ids)) {
                    ...
                    if ($has_ticket_access) {
                        $where_clauses[] = "(t.organization_id IN ($placeholders) OR EXISTS (SELECT 1 FROM ticket_access ta WHERE ta.ticket_id = t.id AND ta.user_id = ?))";
                        foreach ($org_ids as $org_id) {
                            $params[] = $org_id;
                        }
                        $params[] = $current_user['id'];
                    } else {
                        $where_clauses[] = "t.organization_id IN ($placeholders)";
                        foreach ($org_ids as $org_id) {
                            $params[] = $org_id;
                        }
                    }
                } else {
                    // No org scope defined, return no results
                    if ($has_ticket_access) {
                        $where_clauses[] = "EXISTS (SELECT 1 FROM ticket_access ta WHERE ta.ticket_id = t.id AND ta.user_id = ?)";
                        $params[] = $current_user['id'];
                    } else {
                        $where_clauses[] = "0=1";
                    }
    ...
    } elseif ($current_user && $current_user['role'] === 'user') {
        // Regular users see their tickets and shared tickets
        if ($has_ticket_access) {
            $where_clauses[] = "(t.user_id = ? OR EXISTS (SELECT 1 FROM ticket_access ta WHERE ta.ticket_id = t.id AND ta.user_id = ?))";
            $params[] = $current_user['id'];
            $params[] = $current_user['id'];
        } else {
    ...
    $sql = "SELECT s.id, s.name, s.color, COUNT(t.id) as count
            FROM statuses s
            LEFT JOIN tickets t ON t.status_id = s.id AND " . $where . "
            GROUP BY s.id
...
function delete_comment($id) {
    return db_delete('comments', 'id = ?', [$id]);
}
