...
/**
 * Get an active share for a ticket
 */
function get_active_ticket_share($ticket_id) {
    try {
        return db_fetch_one("SELECT * FROM ticket_shares WHERE ticket_id = ? AND is_revoked = 0 AND (expires_at IS NULL OR expires_at > NOW()) ORDER BY created_at DESC LIMIT 1", [$ticket_id]);
    } catch (Exception $e) {
        return null;
    }
}
...
/**
 * Get the latest share record for an organization
 */
function get_latest_report_share($organization_id) {
    try {
        return db_fetch_one("SELECT * FROM report_shares WHERE organization_id = ? ORDER BY created_at DESC LIMIT 1", [$organization_id]);
    } catch (Exception $e) {
        return null;
    }
}
