...
function api_delete_time_entry() {
    ...
    $db->beginTransaction();
    try {
        $undo_token = api_ticket_store_undo_action('time_entry', ['entry' => $entry], (int) $entry['ticket_id']);
...
function api_restore_time_entry() {
    ...
    $action = api_ticket_consume_undo_action('time_entry', trim((string) ($_POST['undo_token'] ?? '')));
    if (!$action) {
    ...
    try {
        ...
        api_success(['message' => t('Time entry restored.'), 'ticket_id' => (int) $entry['ticket_id']]);
    } catch (Throwable $e) {
...
function api_restore_comment() {
    ...
    $action = api_ticket_consume_undo_action('comment', trim((string) ($_POST['undo_token'] ?? '')));
    if (!$action) {
        api_error(t('Undo is no longer available.'), 410);
    }
    ...
    try {
        ...
        api_success(['message' => t('Comment restored.'), 'ticket_id' => (int) $comment['ticket_id']]);
    } catch (Throwable $e) {
        if ($db->inTransaction()) {
            $db->rollBack();
        }
...
function api_delete_attachment() {
    ...
    $attachment = $attachment_id > 0 && function_exists('get_attachment') ? get_attachment($attachment_id) : null;
    if (!$attachment) {
        api_error(t('Attachment not found.'), 404);
    }
    if (!attachment_user_can_delete($attachment, $user)) {
        api_error('Forbidden', 403);
    }
    ...
    try {
        ...
        $db->commit();
        log_activity($ticket_id, (int) $user['id'], 'attachment_deleted', t('Attachment deleted'));
        api_success([
...
function api_restore_attachment(): void
{
    ...
    if (!$ticket || !can_see_ticket($ticket, $user) || !attachment_user_can_delete($attachment, $user)) {
        api_error('Forbidden', 403);
    }
    ...
    try {
        ...
        api_success(['message' => t('Attachment restored.'), 'ticket_id' => (int) $attachment['ticket_id']]);
    } catch (Throwable $e) {
        if ($db->inTransaction()) {
            $db->rollBack();
        }
