...
$ticket_export_params = !empty($ticket['hash']) ? ['t' => $ticket['hash']] : ['id' => $ticket_id];
$ticket_markdown_export_url = url('ticket-export', $ticket_export_params);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['import_worklog_md'])) {
        ...
        try {
            $markdown = ticket_import_read_uploaded_markdown($_FILES['worklog_import_file'] ?? null);
            $payload = ticket_import_parse_markdown($markdown);
            $import_result = ticket_import_append_payload_to_ticket($ticket_id, $payload, $user, [
                'allow_author_override' => is_agent(),
                'default_internal' => !empty($_POST['import_default_internal']),
            ]);

            $message = t('Imported worklog: {comments} comments, {time_entries} time entries.', [
            ...
            if (!empty($warnings)) {
                $message .= ' ' . t('Warnings: {warnings}', ['warnings' => implode(' | ', $warnings)]);
            }

            flash($message, 'success');
        } catch (Throwable $e) {
            flash(t('Failed to import .md file: {error}', ['error' => $e->getMessage()]), 'error');
        }
    ...
    <div class="lg:col-span-3 space-y-3">
        ...
        <div class="card px-2 py-1.5">
            <div class="flex items-center gap-1">
                ...
                <?php if (is_agent()): ?>
                    <a href="<?php echo e($ticket_markdown_export_url); ?>" class="td-tool-btn"
                        title="<?php echo e(t('Download .md')); ?>">
            ...
            <dl class="space-y-3">
                ...
                <div class="flex justify-between">
                    ...
                    <dd class="text-xs" style="color: var(--text-primary);">
                        <?php echo e(get_type_label($ticket['type'])); ?></dd>
                </div>
                ...
                <div class="card card-body">
                    ...
                    <details class="group">
                        ...
                        <div class="pt-3">
                            ...
                            <?php if (can_edit_ticket($ticket, $user) && $can_import_markdown): ?>
                                    <!-- Import Worklog (.md) -->
                                    <div class="pt-2 border-t" style="border-color: var(--border-light);" id="worklog-import">
                                        <label class="form-label-sm mb-1.5">
                                            <?php echo get_icon('upload', 'w-3 h-3 inline mr-1'); ?>                <?php echo e(t('Import worklog (.md)')); ?>
                                        </label>
                                        <p class="text-xs mb-2" style="color: var(--text-muted);">
                                            <?php echo e(t('Upload an English .md worklog template to append comments and time entries.')); ?>
                                        </p>
                                        <form method="post" enctype="multipart/form-data" class="space-y-2">
                                            <?php echo csrf_field(); ?>
                                            <div id="worklog-import-upload-zone" class="upload-zone-compact p-2.5 cursor-pointer">
                                                <input type="file" name="worklog_import_file" id="worklog-import-file-input"
                                                    accept=".md,text/markdown" class="hidden" required>
                                                <div class="flex items-center gap-2" style="color: var(--text-secondary);">
                                                    <?php echo get_icon('file-alt', 'w-3.5 h-3.5 td-text-muted flex-shrink-0'); ?>
                                                    <span class="text-xs">
                                            ...
                                            <p id="worklog-import-file-name" class="hidden text-xs" style="color: var(--text-muted);">
                                            </p>
                                            <label class="inline-flex items-center text-xs" style="color: var(--text-muted);">
                                                <input type="checkbox" name="import_default_internal" value="1" class="mr-1.5 rounded">
                                                <span><?php echo e(t('Default to internal')); ?></span>
                                            </label>
                                            <button type="submit" name="import_worklog_md"
                                                class="btn btn-secondary btn-xs w-full justify-center">
                                                <?php echo get_icon('upload', 'w-3 h-3 mr-1'); ?>                <?php echo e(t('Import from .md')); ?>
                                            </button>
                                        </form>
                                    </div>
                            <?php endif; ?>
...
<script>
    ...
    const initTicketDetailDropzones = function () {
        ...
        const worklogImportFileInput = document.getElementById('worklog-import-file-input');
        const worklogImportFileName = document.getElementById('worklog-import-file-name');
        if (!worklogImportFileInput || !worklogImportFileName) {
            return;
        }

        const updateWorklogImportState = function (files) {
            const currentFiles = files || worklogImportFileInput.files;
            if (!currentFiles || currentFiles.length === 0) {
                worklogImportFileName.textContent = '';
                worklogImportFileName.classList.add('hidden');
                return;
            }
            worklogImportFileName.textContent = currentFiles[0].name;
            worklogImportFileName.classList.remove('hidden');
        };
        if (window.initFileDropzone) {
            window.initFileDropzone({
                zoneId: 'worklog-import-upload-zone',
                inputId: 'worklog-import-file-input',
                acceptedExtensions: ['.md'],
                invalidTypeMessage: '<?php echo e(t('Please select a .md file')); ?>',
                onFilesChanged: updateWorklogImportState
            });
        } else {
            worklogImportFileInput.addEventListener('change', () => updateWorklogImportState(worklogImportFileInput.files));
        }
    };
