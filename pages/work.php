...
$work_period_url = static function (string $period) use ($queue_key, $my_activity_filter, $team_activity_filter, $time_view_scope): string {
    $params = ['period' => $period, 'time_scope' => (string) ($time_view_scope['key'] ?? 'mine')];
    if ($queue_key !== 'mine') {
...
$work_scope_url = static function (string $scope) use ($queue_key, $my_activity_filter, $team_activity_filter, $time_period): string {
    $params = ['period' => (string) ($time_period['period'] ?? 'last_30_days'), 'time_scope' => $scope];
    if (($time_period['period'] ?? '') === 'custom') {
        $params['from_date'] = (string) ($time_period['from_date'] ?? '');
        $params['to_date'] = (string) ($time_period['to_date'] ?? '');
    }
    if ($queue_key !== 'mine') $params['queue'] = $queue_key;
    if (($my_activity_filter['key'] ?? 'last3') !== 'last3') $params['my_activity'] = (string) $my_activity_filter['key'];
    if (($team_activity_filter['key'] ?? 'last3') !== 'last3') $params['team_activity'] = (string) $team_activity_filter['key'];
    return url('work', $params);
};
...
<section id="time-overview" class="fd-card fd-page-section work-overview-card" data-work-time-overview>
    ...
    <div class="work-time-grid">
        ...
        <div class="work-time-metric">
            ...
            <strong><?php echo e(format_duration_minutes((int) ($display_time_totals['selected'] ?? 0))); ?></strong>
        </div>
