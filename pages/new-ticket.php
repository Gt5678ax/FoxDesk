...
$can_import_markdown = can_import_markdown($user);
$organizations = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['import_ticket_md'])) {
        if (!$can_import_markdown) {
            flash(t('You do not have permission to import .md files.'), 'error');
            redirect('new-ticket');
        }

        try {
            $markdown = ticket_import_read_uploaded_markdown($_FILES['ticket_import_file'] ?? null);
            $payload = ticket_import_parse_markdown($markdown);

            $import_organization_id = !empty($_POST['import_organization_id']) ? (int) $_POST['import_organization_id'] : 0;
            if ($import_organization_id > 0 && !in_array($import_organization_id, $allowed_organization_ids, true)) {
                throw new InvalidArgumentException(t('Selected organization is not available.'));
            }

            $import_result = ticket_import_create_ticket_from_payload($payload, $user, [
                'organization_id_override' => $import_organization_id > 0 ? $import_organization_id : null,
                'allowed_organization_ids' => $allowed_organization_ids,
                'allow_requester_override' => is_agent(),
                'allow_author_override' => is_agent(),
                'default_internal' => !empty($_POST['import_default_internal']),
            ]);

            $success_message = t('Imported ticket created: {comments} comments, {time_entries} time entries.', [
                'comments' => (string) ((int) ($import_result['comments'] ?? 0)),
                'time_entries' => (string) ((int) ($import_result['time_entries'] ?? 0)),
            ]);

            $warnings = array_values(array_filter(array_map('trim', (array) ($import_result['warnings'] ?? []))));
            if (!empty($warnings)) {
                $success_message .= ' ' . t('Warnings: {warnings}', ['warnings' => implode(' | ', $warnings)]);
            }

            flash($success_message, 'success');
            redirect('ticket', ['id' => (int) ($import_result['ticket_id'] ?? 0)]);
        } catch (Throwable $e) {
            $error = t('Failed to import .md file: {error}', ['error' => $e->getMessage()]);
        }
    }
...
$import_section_open = isset($_POST['import_ticket_md']);
...
<div class="w-full">
    ...
    <?php if ($can_import_markdown): ?>
        <div class="card card-body mt-4">
            <details <?php echo $import_section_open ? 'open' : ''; ?>>
                <summary class="cursor-pointer list-none flex items-center justify-between gap-3">
                    <div>
                        <h3 class="font-semibold" style="color: var(--text-primary);"><?php echo e(t('Import from Markdown')); ?></h3>
                        <p class="text-xs mt-1" style="color: var(--text-muted);">
                            <?php echo e(t('Import ticket from .md file')); ?>
                        </p>
                    </div>
                    <span class="text-xs" style="color: var(--text-muted);"><?php echo e(t('Expand')); ?></span>
                </summary>

                <div class="mt-4 pt-4 border-t" style="border-color: var(--border-light);">
                    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                        <div>
                            <h4 class="font-medium" style="color: var(--text-primary);"><?php echo e(t('Import ticket from .md')); ?></h4>
                            <p class="text-xs mt-1" style="color: var(--text-muted);">
                                <?php echo e(t('Upload an English template-filled .md file to create a ticket with comments and time entries.')); ?>
                            </p>
                        </div>
                        <?php if (is_admin()): ?>
                            <a href="<?php echo e(url('admin', ['section' => 'settings', 'tab' => 'system'])); ?>" class="text-xs text-blue-600 hover:text-blue-800">
                                <?php echo e(t('Download .md template')); ?>
                            </a>
                        <?php endif; ?>
                    </div>

                    <form method="post" enctype="multipart/form-data" class="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
                        <?php echo csrf_field(); ?>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium mb-1" style="color: var(--text-secondary);"><?php echo e(t('Upload .md file')); ?></label>
                            <div id="ticket-import-upload-zone" class="upload-zone-compact p-3 cursor-pointer">
                                <input type="file" name="ticket_import_file" id="ticket-import-file-input" accept=".md,text/markdown" class="hidden" required>
                                <div class="flex items-center gap-2" style="color: var(--text-secondary);">
                                    <?php echo get_icon('file-alt', 'w-4 h-4 flex-shrink-0'); ?>
                                    <span class="text-sm">
                                        <span class="text-blue-500 font-medium"><?php echo e(t('Click')); ?></span>
                                        <?php echo e(t('or drag .md file')); ?>
                                    </span>
                                </div>
                            </div>
                            <p id="ticket-import-file-name" class="hidden text-xs mt-1" style="color: var(--text-muted);"></p>
                        </div>
                        <?php if (!empty($organizations)): ?>
                            <div>
                                <label class="block text-sm font-medium mb-1" style="color: var(--text-secondary);"><?php echo e(t('Company')); ?></label>
                                <select name="import_organization_id" class="form-select">
                                    <option value=""><?php echo e(t('-- No organization --')); ?></option>
                                    <?php foreach ($organizations as $org): ?>
                                        <option value="<?php echo (int) $org['id']; ?>" <?php echo $default_organization_id === (int) $org['id'] ? 'selected' : ''; ?>>
                                            <?php echo e($org['name']); ?>
                        ...
                        <?php endif; ?>
                        <div class="md:col-span-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                            <label class="inline-flex items-center text-sm" style="color: var(--text-secondary);">
                                <input type="checkbox" name="import_default_internal" value="1" class="mr-2 rounded">
                                <span><?php echo e(t('Default imported comments to internal')); ?></span>
                            </label>
                            <button type="submit" name="import_ticket_md" class="btn btn-secondary">
                                <?php echo get_icon('upload', 'mr-1'); ?><?php echo e(t('Import from .md')); ?>
                            </button>
                        </div>
                    </form>
                </div>
...
<script>
    ...
    const initTicketUploadZones = function () {
        ...
        const ticketImportFileInput = document.getElementById('ticket-import-file-input');
        const ticketImportFileName = document.getElementById('ticket-import-file-name');
        if (!ticketImportFileInput || !ticketImportFileName) {
            return;
        }

        const updateImportFileState = function (files) {
            const currentFiles = files || ticketImportFileInput.files;
            if (!currentFiles || currentFiles.length === 0) {
                ticketImportFileName.textContent = '';
                ticketImportFileName.classList.add('hidden');
                return;
            }
            ticketImportFileName.textContent = currentFiles[0].name;
            ticketImportFileName.classList.remove('hidden');
        };
        if (window.initFileDropzone) {
            window.initFileDropzone({
                zoneId: 'ticket-import-upload-zone',
                inputId: 'ticket-import-file-input',
                acceptedExtensions: ['.md'],
                invalidTypeMessage: '<?php echo e(t('Please select a .md file')); ?>',
                onFilesChanged: updateImportFileState
            });
        } else {
            ticketImportFileInput.addEventListener('change', () => updateImportFileState(ticketImportFileInput.files));
        }
    };
