...
if ($due_date_filter !== '') {
    if ($due_date_filter === 'overdue') {
        ...
        if ($due_dt) {
            $due_date_value = $due_dt->format('Y-m-d');
    ...
    <?php else: ?>
        ...
        <div class="block lg:hidden">
            <?php foreach ($ticket_groups as $group): ?>
                ...
                $is_overdue_mobile = !empty($ticket['due_date']) && strtotime($ticket['due_date']) < time();
                ?>
                <div class="p-4 ticket-list-item<?php echo $is_overdue_mobile ? ' ticket-overdue' : ''; ?>" style="border-left: 3px solid <?php echo e($ticket['status_color']); ?>;">
                            ...
                            <a href="<?php echo ticket_url($ticket); ?>" class="flex-1 min-w-0">
                                ...
                                <div class="text-sm mt-1 flex flex-wrap items-center gap-2" style="color: var(--text-muted);">
                                    ...
                                    <?php if (!empty($ticket['due_date'])): ?>
                                        ...
                                        $is_overdue = $due_ts < time() && $ticket['status_id'] != 3; // Assuming 3 is closed, ideally check is_closed flag if available
                                        // Better check: $ticket['status_type'] === 'closed' if available, but for now we look at date
                                        ?>
                                        <span
            ...
            <table class="w-full hidden lg:table tickets-table text-xs" style="table-layout: fixed;">
                    ...
                    <tr class="border-b" style="border-color: var(--border-light);">
                        <th class="px-3 py-2.5 text-left" style="width: 80px;">
                            <div class="flex items-center gap-1">
                                ...
                                <?php endif; ?>
                                <span class="text-[10px] font-medium uppercase tracking-wider" style="color: var(--text-muted);">ID</span>
                            </div>
                        </th>
                ...
                <?php foreach ($ticket_groups as $group): ?>
                    ...
                    <?php foreach ($group['tickets'] as $ticket):
                        $priority_name = $ticket['priority_name'] ?? get_priority_label($ticket['priority_id'] ?? $ticket['priority'] ?? 'medium');
                        $priority_color = $ticket['priority_color'] ?? get_priority_color($ticket['priority_id'] ?? $ticket['priority'] ?? 'medium');
                        $is_overdue = !empty($ticket['due_date']) && strtotime($ticket['due_date']) < time();
                        ?>
                        <tr class="ticket-row<?php echo $is_overdue ? ' ticket-overdue' : ''; ?>" style="border-left: 3px solid <?php echo e($ticket['status_color']); ?>;" data-href="<?php echo e(ticket_url($ticket)); ?>">
                            <td class="px-3 py-2.5 whitespace-nowrap align-top">
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <?php endif; ?>
                                    <div>
                                        <a href="<?php echo ticket_url($ticket); ?>" class="ticket-code" title="<?php echo e('#' . (int) $ticket['id']); ?>">
                                            <?php echo e(get_ticket_code($ticket['id'])); ?>
                                        </a>
                                        <div class="text-[10px]" style="color: var(--text-muted);"><?php echo date('d.m', strtotime($ticket['created_at'])); ?></div>
                                    </div>
                                </div>
                            </td>
...
<script>
    ...
    // Clickable rows - navigate on click (skip checkbox, links, buttons)
    document.querySelectorAll('tr[data-href]').forEach(function(row) {
        row.addEventListener('click', function(e) {
            if (e.target.closest('a, button, input, select, .bulk-checkbox')) return;
